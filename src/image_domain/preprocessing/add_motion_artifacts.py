#!/usr/bin/env python3
"""
Module: Motion Artifact Simulation in Image Domain (3D Sinograms)
...
from config.config import CLEAN_SINOGRAM_ROOT, ARTIFACT_SINOGRAM_ROOT
...
def main():
    ...
    sample = np.load(clean_files[0])
    print(f"Sample sinogram shape: {sample.shape}, dtype: {sample.dtype}")
    ...
    for clean_path in tqdm(clean_files, desc="Generating artifact sinograms"):
        ...
        if sino_clean.ndim != 3:
            print(f"[WARNING] Skipping {clean_path.name} — invalid shape")
            num_skipped += 1
            continue
