#!/usr/bin/env python3
"""
Generates MPR comparison figures (Clean | Artifact | MR-LKV) WITH ROI zoom for Projection domain
"""
...
OUT_DIR = Path(__file__).resolve().parent / "results" / "mpr_all_roi"
...
def find_volume(folder: str, patient_id: str):
    folder_path = CT_ROOT / folder

    if not folder_path.exists():
        print(f"Folder missing: {folder_path}")
        return None

    #Extract base ID (important fix)
    base_id = patient_id.split(" ")[0]

    # Try both naming styles
    candidates = [
        folder_path / f"{patient_id}.npy",           # full name
        folder_path / f"{base_id}.npy",              # simple name
        folder_path / f"{base_id} {base_id}.npy",    # duplicated name
    ]

    for p in candidates:
        if p.exists():
            print(f"Loaded {folder}: {p.name}")
            return np.load(p).astype(np.float32)

    print(f"Missing {folder}: {patient_id}")
    return None
...
def generate_mpr_figure(patient_id: str, clean_folder: str = "clean"):
    ...
    # Use MR-LKV for improvement metric (or change if needed)
    ref_pred = volumes.get("mr_lkv")

    severity = compute_artifact_severity(clean_vol, artifact_vol) if artifact_vol is not None else 0
    improvement = compute_improvement(clean_vol, artifact_vol, ref_pred) if ref_pred is not None else 0
    ...
    plt.suptitle(
        f"{patient_id.split()[0]} | Severity: {severity:.4f} | MR-LKV Improvement: {improvement:+.4f}",
        fontsize=12
    )

    plt.tight_layout()

    out_path = OUT_DIR / f"{patient_id.split()[0]}_comparison.png"
